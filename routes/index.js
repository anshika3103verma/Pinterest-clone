  ...
  try {
    ...
    try {
      const [nature, travel, food, fashion] = await Promise.all([
        fetchJSON(`https://api.unsplash.com/photos/random?count=20&topic=nature&client_id=-p23DG2BaSw-hntVeQpFbdUypqmBdUIDucKittz1NtM`),
        fetchJSON(`https://api.unsplash.com/photos/random?count=20&topic=travel&client_id=-p23DG2BaSw-hntVeQpFbdUypqmBdUIDucKittz1NtM`),
        fetchJSON(`https://api.unsplash.com/photos/random?count=20&topic=food-drink&client_id=-p23DG2BaSw-hntVeQpFbdUypqmBdUIDucKittz1NtM`),
        fetchJSON(`https://api.unsplash.com/photos/random?count=20&topic=fashion&client_id=-p23DG2BaSw-hntVeQpFbdUypqmBdUIDucKittz1NtM`),
      ]);
...
router.get('/search', isLoggedIn, async function(req, res) {
  try {
    ...
    let unsplashPosts = [];
    try {
      const data = await fetchJSON(`https://api.unsplash.com/search/photos?query=${query}&per_page=20&client_id=-p23DG2BaSw-hntVeQpFbdUypqmBdUIDucKittz1NtM`);
      unsplashPosts = data.results || [];
    } catch(err) {
      console.log("Unsplash search failed:", err.message);
    }

    res.render('feed', { user, posts: userPosts, unsplashPosts, searchQuery: query });
  } catch(err) {
